...
class ElasticsearchInstance(object):
    def __init__(self):
        ...
        args["connections_per_node"] = 1000
        ssl = True
        if ssl:
            args["verify_certs"] = self.str_to_bool(os.getenv("DB_VERIFY_CERTS", "false"))
        ...
        self._index = os.getenv("DB_INDEX", "oceandb")
        try:
            self._es = Elasticsearch(f"{scheme}://{host}:{port}", **args)
            while not self._es.ping():
    ...
    def read_did_state(self, did):
        ...
        :return: object value from elasticsearch.
        """
        return self.es.get(index="log-app-compute-to-data--8-w2023.42", id=did)["_source"]